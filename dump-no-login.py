...
class GroupNoLogin():
    ...
    def LandingGroup(self):
        url = 'https://www.facebook.com/groups/175107520598383' #--> Change Facebook Group URL
        self.data_post, self.data_grup = self.ScrapDataGroup(url)
        self.ScrapOverall()
    
    #--> Scrap Data Group For Query
    def ScrapDataGroup(self,url):
    ...
    #--> Get Overall Info
    def ScrapOverall(self,cursor=None):
        ...
        #--> Submit To Comment
        with ThreadPoolExecutor(max_workers=35) as TPE:
            for fbid in feedback: TPE.submit(self.ScrapComment,fbid)

        #--> Submit To React
        with ThreadPoolExecutor(max_workers=35) as TPE:
            for fbid in feedback: TPE.submit(self.ScrapReact,fbid)

        #--> Loop
        try: self.ScrapOverall(re.search('"end_cursor":"(.*?)"',str(pos)).group(1))
    ...
    def ScrapPost(self,dat):
        ...
        if 'pfbid' in ray['id']: pass
            ...
            else: self.result.append(ray); print('%s|%s'%(ray['id'],ray['name']))
...
class SearchName():
    ...
    def Scrap(self,nama):
        ...
        with ThreadPoolExecutor(max_workers=30) as TPE:
            for id in kid: TPE.submit(self.GenerateID,id)
        try:
