...
class main:
    ...
    def insert_cookie(self):
        ...
        token_eaag = self.generate_token_eaag(ciko)
        token_eaab = self.generate_token_eaab(ciko)
        try:os.mkdir("login")
        except:pass
        open('login/cookie.json','w').write(ciko)
        open('login/token_eaag.json','w').write(token_eaag)
        open('login/token_eaab.json','w').write(token_eaab)
        self.cek_cookies()
...
class main_dump:
    ...
    def friendlist(self):
        self.tamp3 = []
        url = 'https://graph.facebook.com/me?fields=friends.fields(id,name)&limit=5000&access_token=%s'%(self.token_eaag)
        req = self.xyz.get(url,cookies=self.cookie).json()
        try:
